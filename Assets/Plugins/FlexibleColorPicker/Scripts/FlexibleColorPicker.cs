...
public class FlexibleColorPicker : MonoBehaviour
{
	...
	*/
    ...
    public bool InputSlected => rInput.isFocused || gInput.isFocused || bInput.isFocused || aInput.isFocused;
	...
	*/
    ...
    private void OnEnable()
    {
        ...
        _intToString = new string[256];

        _intToString[0] = "";

        for (int i = 1; i < _intToString.Length; i++)
        {
    ...
    /// Softly sanitize hex color input and apply it
    /// </summary>
    public void TypeColor(string input)
    {
        TypeHex(input, false);
	...
	*/
    ...
    private BufferedColor GetDefaultColor(PickerType type)
    {
        switch (type)
        {
            case PickerType.Main: return GetDefaultColor();
    ...
    private BufferedColor GetDefaultColor()
    {
	...
	*/
    ...
    {
        rInput.text = _intToString[(int)(this.Color.r * 255)];
        gInput.text = _intToString[(int)(this.Color.g * 255)];
        bInput.text = _intToString[(int)(this.Color.b * 255)];
        aInput.text = _intToString[(int)(this.Color.a * 255)];
    }
    ...
    private void TypeHex(string input, bool finish)
    {
        ...
        ColorUtility.TryParseHtmlString(parseText, out Color newColor);
        this.bufferedColor.Set(newColor);
        UpdateMarkers();
        UpdateTextures();
    }

    private string FinishTypeColor(string input, InputType type)
    {
        int colorValue;
        if (input == "")
        {
        ...
        {
            colorValue = Mathf.Clamp(int.Parse(input), 0, 255);
        }

        Color newColor = this.bufferedColor.color;
        switch (type)
        {
            case InputType.r:
                newColor.r = colorValue / 255.0f;
                break;
            case InputType.g:
                newColor.g = colorValue / 255.0f;
                break;
            case InputType.b:
                newColor.b = colorValue / 255.0f;
                break;
            case InputType.a:
                newColor.a = colorValue / 255.0f;
                break;
        }
        this.bufferedColor.Set(newColor);
        UpdateMarkers();
        ...
        return _intToString[colorValue];
    }

    public void FinishInputColor()
    {
        if (!InputSlected)
        {
            return;
        ...
        rInput.text = FinishTypeColor(rInput.text, InputType.r);
        gInput.text = FinishTypeColor(gInput.text, InputType.g);
        bInput.text = FinishTypeColor(bInput.text, InputType.b);
        aInput.text = FinishTypeColor(aInput.text, InputType.a);
    }
