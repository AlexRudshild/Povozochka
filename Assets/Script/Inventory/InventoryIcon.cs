...
public class InventoryIcon : MonoBehaviour
{
    ...
    public bool IconBlock
    {
        ...
        set
        {
            _iconBlock = value; if (_iconBlock)
            {
	...
	private IEnumerator ClickEvent()
	{
		...
		{
			SetBackImage(false);
			InventoryControl.Instance.BeginDrag(this);
		}
		yield break;
	}
    ...
    {
		InventoryControl.Instance.DoubleClick(this);
    }
	...
	public void UpdateCells()
	{
		...
		for (int y = 0; y < iconHeight; y++)
		{
			for (int x = 0; x < iconWidth; x++)
			{
				if (Item.Cells[y, x])
				{
					var curElement = Instantiate(InventoryControl.Instance.PointPrefab, transform).GetComponent<RectTransform>();
